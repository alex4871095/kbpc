...
str = sw1_show_cdp_neighbors_detail + r1_show_cdp_neighbors_detail + r2_show_cdp_neighbors_detail + r3_show_cdp_neighbors_detail + r4_show_cdp_neighbors_detail + r5_show_cdp_neighbors_detail
...
def str_to_list( neigh_string ):
	list_neigh = neigh_string.split()
	for i in range(len(list_neigh)):
		y = list_neigh[i]
		z = y.strip('>show')
		list_neigh[i] = z
	list = ['']
	...
	i = 1
	#print list_neigh
	z = list_neigh.index('Device')
	list.append(list_neigh[z+9])
	z += 9
	while z <= len(list_neigh):
		if z+10 >= len(list_neigh):
			break
		z += 10
		list.append(list_neigh[z])
		#print list
	return list

s1_neigh = str_to_list(sw1_show_cdp_neighbors)
r1_neigh = str_to_list(r1_show_cdp_neighbors)
r2_neigh = str_to_list(r2_show_cdp_neighbors)
r3_neigh = str_to_list(r3_show_cdp_neighbors)
r4_neigh = str_to_list(r4_show_cdp_neighbors)
r5_neigh = str_to_list(r5_show_cdp_neighbors)
...
for i in list:
	try:
		...
		network_devices[key] = {}
		try:
			y = list.index('address:')
			z = list.pop(y)
			network_devices[key]['ip'] = list[y]
                except ValueError:
                        print "err"
                        pass    
		try:
                        y = list.index('Platform:')
                        z = list.pop(y)
                        network_devices[key]['vendor'] = list[y]
			network_devices[key]['model'] = list[y+1]
                except ValueError:
                        print "err"
                        pass    
		try:
                        y = list.index('Capabilities:')
                        z = list.pop(y)
                        network_devices[key]['device_type'] = list[y]
		except ValueError:
			print "err"
			pass	
	except ValueError:
...
if network_devices.get('SW1'):
        network_devices['SW1']['adjacent_devices'] = s1_neigh[1:len(s1_neigh)]
if network_devices.get('R1'):
        network_devices['R1']['adjacent_devices'] = r1_neigh[1:len(r1_neigh)]
if network_devices.get('R2'):
        network_devices['R2']['adjacent_devices'] = r2_neigh[1:len(r2_neigh)]
if network_devices.get('R3'):
        network_devices['R3']['adjacent_devices'] = r3_neigh[1:len(r3_neigh)]
if network_devices.get('R4'):
        network_devices['R4']['adjacent_devices'] = r4_neigh[1:len(r4_neigh)]
if network_devices.get('R5'):
        network_devices['R5']['adjacent_devices'] = r5_neigh[1:len(r5_neigh)]

for i in sorted(network_devices.keys()):
	print i, ":"
	for y in network_devices[i].keys():
		print y, ":", network_devices[i][y]
